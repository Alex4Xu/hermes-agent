...
class WeChatAdapter(BasePlatformAdapter):
    ...
    def _stop_polling_orphan(self, account_id: str) -> None:
        """Remove a stale account ID from the index and cancel its poll task."""
        task = self._poll_tasks.pop(account_id, None)
        if task and not task.done():
            task.cancel()
        self._poll_longpoll_timeout_ms.pop(account_id, None)
        self._paused_until.pop(account_id, None)
        # Clean up index entry so future connect() won't restart it
        try:
            ids = self._state.list_account_ids()
            if account_id in ids:
                ids.remove(account_id)
                from .wechat_state import _atomic_write_json
                _atomic_write_json(self._state.index_path, ids)
        except Exception as exc:
            logger.error("[%s] Failed to remove orphan account %s from index: %s", self.name, account_id, exc)
    ...
    async def _poll_account_once(self, account_id: str) -> None:
        account = self._state.load_account(account_id)
        if not account:
            logger.warning("[%s] WeChat account %s no longer exists — removing from index and stopping poll", self.name, account_id)
            self._stop_polling_orphan(account_id)
            return
